...
Route::group(['before' => 'authenticate'], function()
{
    ...
    Route::post('image_upload', function() {
        ...
        if ((($mime == 'image/gif')
            ...
            move_uploaded_file($_FILES['file']['tmp_name'], getcwd().'/download/'.$name);

            // Generate response.
            $response = new StdClass;
            $response->link = asset('/download/'.$name);
    ...
    Route::post('delete_image', function()
    {
        ...
        if (file_exists(getcwd().'/download/'.$src)) {
            // Delete file.
            unlink(getcwd().'/download/'.$src);
        }
    });
});
